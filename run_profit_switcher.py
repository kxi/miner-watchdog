...
# def run_miner(cmd_name, cmd_path):
    # miner_proc = subprocess.Popen("python miner_watchdog.py raven"
    #                         stdout = subprocess.PIPE, stderr = subprocess.PIPE, shell=True)
    # miner_stdout = miner_proc.stdout
    print("Here")
    try:
        miner_proc = subprocess.call("python miner_watchdog.py raven", stdout = subprocess.PIPE, stderr = subprocess.PIPE,)
...
def kill_miner():
    proc = subprocess.Popen("taskkill /im miner_watchdog.py /f", stdout = subprocess.PIPE, stderr = subprocess.PIPE, shell=True)

def main():

    timeout = 300
    hostname = sys.argv[1]
    switch_count = int(sys.argv[2]) * 1 # 15 Min = 90 Count
    ...
    thread = Thread(target=run_miner)
    thread.daemon = True
    thread.start()
